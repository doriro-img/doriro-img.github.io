// 웹페이지 화면 컷 — 카드 단위로 잘라 올해/이번달 폴더에 저장한다.
// 전체 페이지 캡처는 세로가 너무 길어 본문 폭에 맞추면 글씨가 안 보인다.
// 실행: node make-shots.cjs
// 찍을 사이트는 SHOT_BASE 로 바꾼다. 아래 SHOTS 에 대상 화면을 적는다.
...
const BASE = process.env.SHOT_BASE || 'https://duostockledger.github.io';
...
// 찍을 화면 목록. tab을 누른 뒤 card 제목이 있는 영역만 잘라낸다
const SHOTS = [
...
(async () => {
  ...
  console.log('이어서 `node push.cjs "설명"` 으로 올리세요');
})();
